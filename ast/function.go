...
func MatchFunction(fn *ast.FuncDecl, signature *FunctionSignature) bool {
	// 检查参数数量
	if fn.Type.Params == nil || len(fn.Type.Params.List) != len(signature.Params) || fn.Type.Results == nil || len(fn.Type.Results.List) != len(signature.Results) {
		return false
	}

	if signature.NamePattern != nil {
		if !signature.NamePattern.MatchString(fn.Name.Name) {
			return false
		}
	}
	...
	for i, param := range fn.Type.Params.List {
		if paramTypeName := GetExprTypeName(param.Type); paramTypeName != signature.Params[i] {
			return false
		}
	}

	for i, result := range fn.Type.Results.List {
		if resultTypeName := GetExprTypeName(result.Type); resultTypeName != signature.Results[i] {
			return false
		}
	}
...
// GetReceiverTypeName 获取函数的receiver类型, 如果ignorePointer为true, 则去除前面的*号
...
func GetReceiverTypeName(fn *ast.FuncDecl, ignorePointer ...bool) string {
	...
	v := GetExprTypeName(fn.Recv.List[0].Type)
...
func InspectFunction(srcPath string, fnParams, fnResults []string, annotationPrefix string) ([]*Function, error) {
	functions, err := getFunctions(srcPath, fnParams, fnResults)
	if err != nil {
		return nil, err
	}

	// 默认从moduleSrcPath目录开始解析, e,g: src/base/Package/service
	funcInfos := make([]*Function, 0)
	for _, fn := range functions {
		annotations, plainComments, err := parseComments(fn.Comments, annotationPrefix)
		if err != nil {
			return nil, err
		}

		funcInfos = append(funcInfos, &Function{
...
func getFunctions(srcPath string, fnParams, fnResults []string) ([]astRawFunction, error) {
...
// e,g:
// xxxHandler(ctx context.Context, event *common.InvocationEvent) (*common.Content, panic)
